...
public class MemberService {
    ...
    public MemberResponse createMember(MemberCreateRequest request, MultipartFile file) {
        ...
        try {
            ...
            // 나중에 프론트에서 회우너 정보를 요청할 때 url도 같이 전달 -> 프론트에서 <img> 등으로 요청을 보낼 겁니다.

            // 만약 presigned url을 사용한다면 굳이 DB에 url을 저장할 필요가 없음
            // 파일명(객체 Key)을 DB에 저장, 그리고 데이터를 불러올 일이 있다면 그 때마다
            // presigned url을 생성
